...
PATH = "/home/makhataei/Projects/CallCanterQC/datasets/DushaEmotionAudio"
# train_df = pd.read_csv("/media/makhataei/Backups/555/data_train.csv", on_bad_lines='skip')
# a=train_df.values
a = os.listdir(PATH)

for folder in a:
	b = os.listdir(f"{PATH}/{folder}")
	for folderr in b:
		c = os.listdir(f"{PATH}/{folder}/{folderr}")
		for file in c:
			with open(f'tester.csv', 'a') as fileee:
				fileee.writelines(
					# f'{files}-{filess}, {bb},{ent[0]},{ent[1]},{ent[2]},{ent[3]},{ent[4]},{ent[5]},{ent[6]},{ent[7]}\n')
					f'{folder}/{file.split(".")[0]}.wav,{folderr}\n')
